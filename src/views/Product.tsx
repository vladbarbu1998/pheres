...
export default function ProductPage() {
  const { categorySlug, productSlug } = (useParams() as unknown as { categorySlug: string; productSlug: string });
  ...
  useEffect(() => {
    if (product?.product_type === "couture") {
      ...
      if (coutureCollection) {
        router.replace(`/couture/${coutureCollection.slug}/${product.slug}`);
      }
    }
  }, [product, router]);
