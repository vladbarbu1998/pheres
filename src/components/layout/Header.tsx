...
export function Header() {
  ...
  const handleAccountClick = () => {
    if (user) {
      ...
      router.push(`/account/login?redirect=${encodeURIComponent("/account")}`);
    }
  };
  ...
  const handleFavoritesClick = () => {
    if (user) {
      ...
      router.push(`/account/login?redirect=${encodeURIComponent("/account/favorites")}`);
    }
  };
            ...
            {navigation.map((item) =>
              item.hasMegaMenu ? (
                <CollectionsMegaMenuDesktop
                  key={item.name}
                  isActive={(pathname?.startsWith("/collections") ?? false) || (pathname?.startsWith("/shop/collection") ?? false)}
                />
              ) : (
                <Link
                  key={item.name}
